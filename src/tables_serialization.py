...
class TqdmLoggingHandler(logging.Handler):
    def emit(self, record):
        """Emit a log record to the message queue for tqdm display.
...
def process_messages():
    """Process and display queued log messages using tqdm."""
...
class TableSerializer(BaseOpenaiProcessor):
    def __init__(self, preserve_temp_files: bool = True):
        """Initialize the TableSerializer with OpenAI processor and logging.

        Args:
            preserve_temp_files (bool): Whether to keep temporary request/result files. Defaults to True.
        """
        super().__init__()
    ...
    def _get_table_context(self, json_report, target_table_index):
        """Extract text context before and after a specific table in a JSON report.

        Args:
            json_report (dict): The JSON report containing tables and content.
            target_table_index (int): The ID of the target table.
    ...
    def _send_serialization_request(self, table, context_before, context_after):
        """Send a table serialization request to the OpenAI API.

        Args:
            table (str): HTML content of the table.
            context_before (str): Text context before the table.
            context_after (str): Text context after the table.

        Returns:
            dict: Serialization result from the API.
        """
        user_prompt = ""
        if context_before:
    ...
    def _serialize_table(self, json_report: dict, target_table_index: int) -> dict:
        """Serialize a single table with its context.
        ...
        Returns:
            dict: Serialization result for the table.
        """
    ...
    def serialize_tables(self, json_report: dict) -> dict:
        ...
        Returns:
            dict: Updated JSON report with serialized tables.
        """
    ...
    async def async_serialize_tables(
        ...
        Args:
            json_report (dict): The JSON report containing tables and content.
            requests_filepath (str): Path to store temporary request logs. Defaults to './temp_async_llm_requests.jsonl'.
            results_filepath (str): Path to store temporary result logs. Defaults to './temp_async_llm_results.jsonl'.
    ...
    def process_file(self, json_path: Path) -> None:
        """Process a single JSON file and serialize its tables.
    ...
    def process_directory_parallel(self, input_dir: Path, max_workers: int = 5):
        ...
        Args:
            input_dir (Path): Path to directory containing JSON files.
            max_workers (int): Maximum number of threads to use. Defaults to 5.
        """
