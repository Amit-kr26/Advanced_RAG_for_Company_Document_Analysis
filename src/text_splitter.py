...
class TextSplitter:
    def _get_serialized_tables_by_page(self, tables: List[Dict]) -> Dict[int, List[Dict]]:
        ...
        Args:
            tables (List[Dict]): List of table dictionaries containing serialization data.

        Returns:
            Dict[int, List[Dict]]: Dictionary mapping page numbers to lists of table metadata.
        """
        tables_by_page = {}
        for table in tables:
    ...
    def _split_report(self, file_content: Dict[str, any], serialized_tables_report_path: Optional[Path] = None) -> Dict[str, any]:
        ...
        Args:
            file_content (Dict[str, any]): The JSON report content to split.
            serialized_tables_report_path (Optional[Path]): Path to the report with serialized tables. Defaults to None.

        Returns:
            Dict[str, any]: Updated report with chunked content.
        """
        chunks = []
        chunk_id = 0
        tables_by_page = {}
    ...
    def count_tokens(self, string: str, encoding_name="o200k_base"):
        """Count the number of tokens in a string using a specified encoding.

        Args:
            string (str): The text to tokenize.
            encoding_name (str): The name of the token encoding. Defaults to "o200k_base".
    ...
    def _split_page(self, page: Dict[str, any], chunk_size: int = 300, chunk_overlap: int = 50) -> List[Dict[str, any]]:
        ...
        Args:
            page (Dict[str, any]): The page dictionary containing text and page number.
            chunk_size (int): Target size of each chunk in tokens. Defaults to 300.
            chunk_overlap (int): Number of overlapping tokens between chunks. Defaults to 50.

        Returns:
            List[Dict[str, any]]: List of chunk dictionaries with page, token length, and text.
        """
    ...
    def split_all_reports(self, all_report_dir: Path, output_dir: Path, serialized_tables_dir: Optional[Path] = None):
        """Split all JSON reports in a directory into chunks and save to output directory.

        Args:
            all_report_dir (Path): Directory containing JSON reports to split.
            output_dir (Path): Directory to save chunked reports.
            serialized_tables_dir (Optional[Path]): Directory containing reports with serialized tables. Defaults to None.
        """
